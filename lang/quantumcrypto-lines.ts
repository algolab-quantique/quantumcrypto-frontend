...
export const quantumcryptoLines: LanguageItem[] = [
    {
        ...
        'component.dps.about.translationToCome' : 'English translation in progress...',
    ...
    },
    {
        ...
        'component.e91.about.bellInequalities': 'Les inégalités de Bell sont un ensemble de relations mathématiques qui doivent toujours être satisfaites si la nature obéit aux lois de la physique classique et à l\'idée de variables cachées locales (c\'est-à-dire que les résultats de mesure sont prédéterminés et qu\'aucune information ne voyage plus vite que la lumière). Cependant, la mécanique quantique prédit — et les expériences confirment — que certains systèmes intriqués, comme les paires de Bell, peuvent violer ces inégalités : ils présentent des corrélations impossibles à expliquer par une théorie classique. Tester la violation des inégalités de Bell prouve donc l\'existence de l\'intrication quantique et exclut toute explication par des variables cachées locales.',

        'component.e91.about.chshInequality.title': 'Inégalité CHSH',
        'component.e91.about.chshInequality': 'L\'inégalité CHSH (Clauser, Horne, Shimony, Holt) est une version spécifique et expérimentalement testable des inégalités de Bell. Elle s\'applique aux mesures sur deux qubits intriqués, chacun mesuré dans deux bases différentes. Selon la physique classique, la valeur du paramètre S calculé à partir des corrélations mesurées ne peut pas dépasser 2. Cependant, la mécanique quantique permet une valeur maximale de 2√2, prouvant ainsi la présence d\'intrication et l\'absence de variables cachées locales. Dans le protocole E91, vérifier cette inégalité assure la sécurité de la clé générée.',
    ...
    },
    {
        ...
        'component.e91.about.encryptionKey': 'Una clave de cifrado es una información secreta, generalmente una secuencia de bits (0 y 1), compartida únicamente entre dos partes (como Alice y Bob). Permite, a través de un algoritmo criptográfico, transformar un mensaje legible (texto plano) en un mensaje cifrado (ilegible para otros), y luego descifrarlo. A diferencia de una contraseña, la clave no está destinada a ser memorizada por un humano, sino a ser utilizada por un programa informático. El objetivo de protocolos como E91 o BB84 es generar y compartir esta clave de manera perfectamente segura, para que solo Alice y Bob puedan usarla para proteger sus comunicaciones.',

        'component.e91.about.publicPrivate.title': 'Canal público (y privado)',
        'component.e91.about.publicPrivate': 'Un canal público es un medio de comunicación (como Internet, una fibra óptica o una línea telefónica) donde cualquiera puede potencialmente interceptar o escuchar los mensajes intercambiados, como hablar en voz alta en una habitación llena de gente. Por el contrario, un canal privado garantiza que solo las partes involucradas puedan acceder a la comunicación, como una conversación en voz baja apartada. En la práctica, es difícil garantizar la confidencialidad de un canal: por eso se utiliza la criptografía para hacer incomprensibles los mensajes en un canal público. En los protocolos de criptografía cuántica (BB84, E91, DPS, etc.), la seguridad se basa en la física cuántica, no en la confidencialidad del canal.',
        ...
        'component.e91.about.bellInequalities': 'Las desigualdades de Bell son un conjunto de relaciones matemáticas que siempre deben satisfacerse si la naturaleza obedece las leyes de la física clásica y la idea de variables ocultas locales (es decir, que los resultados de medición están predeterminados y ninguna información viaja más rápido que la luz). Sin embargo, la mecánica cuántica predice —y los experimentos confirman— que ciertos sistemas entrelazados, como los pares de Bell, pueden violar estas desigualdades: exhiben correlaciones imposibles de explicar por una teoría clásica. Probar la violación de las desigualdades de Bell demuestra así la existencia del entrelazamiento cuántico y descarta cualquier explicación por variables ocultas locales.',
