...
/** Difficulty rating (1-3 QC lock icons) per protocol */
...
const QCLockIcon = ({ active }: { active: boolean }) => (
    <span className="relative inline-block w-[20px] h-[20px]">
        ...
        <Image
            ...
            width={20}
            height={20}
            className={`block dark:hidden ${active ? 'opacity-100' : 'opacity-20'}`}
        />
        ...
        <Image
            ...
            width={20}
            height={20}
            className={`hidden dark:block ${active ? 'opacity-100' : 'opacity-20'}`}
        />
    </span>
);

const DifficultyLocks = ({ level }: { level: number }) => (
    <div className="flex items-center gap-1.5" title={`Difficulty ${level}/3`}>
        {[1, 2, 3].map((i) => (
            <QCLockIcon key={i} active={i <= level} />
        ))}
    </div>
);

const ProtocolsSectionV2Locks = () => {
                                    ...
                                    <div className="group hover:scale-[1.03] cursor-pointer transition-all duration-200
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <DifficultyLocks level={meta.difficulty} />
                                            </div>
...
export default ProtocolsSectionV2Locks;
