...
/**
 * Multi: build solo's `{drawn, detected}` record from a backend that RECORDS
 * the detection (E91 does, BB84 does not — that one uses `deriveRoomEveStory`).
 * Multi results then read exactly like solo's.
 *
 * `|| eve_detected` is not tidiness: E91's backend keeps ONE iteration per room
 * and `RESTART_WITHOUT_EVE` sets `eve_present = False` on it, so a student who
 * catches Eve leaves a record saying she was never there.
 */
export const deriveRoomEveRecord = (iterations: RoomIteration[]): EveOutcome => ({
    drawn: iterations.some(
        iteration => !!iteration?.eve_present || !!iteration?.eve_detected),
    detected: iterations.some(iteration => !!iteration?.eve_detected),
});
